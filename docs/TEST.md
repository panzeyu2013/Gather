...
### 3.2 参数调节
- [ ] Threshold 滑块范围 0–30，拖动时数值实时更新
- [ ] Min Group Size 输入框可输入 2–10
- [ ] 滑块和输入框互不干扰
- [ ] 拖动 Threshold 命中预计算档位（T±4/T±8）时分组**即时切换**，无需点击重新聚类
- [ ] 命中档位切换后，Min Group Size / 分组模式草稿不被旧档结果覆盖
...
- 分析进度按照片持久化：人脸/相似度分析中断后可**续跑**（仅重算未完成照片，
  checksum 补齐为独立后台任务 `checksum.backfill`，重启自动恢复）
